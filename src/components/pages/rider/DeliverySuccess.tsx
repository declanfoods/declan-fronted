...
export default function DeliverySuccess() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (deliveryRes.status === 'fulfilled') setDelivery(deliveryRes.value.data.data.delivery);
      if (metricsRes.status === 'fulfilled') setMetrics(metricsRes.value.data.data);
      if (deliveriesRes.status === 'fulfilled') {
        const next = (deliveriesRes.value.data.data.deliveries ?? []).find(
          (d) => d.id !== id && d.status === 'ASSIGNED'
        );
  ...
  const todaysTotal = num(metrics, 'todaysEarnings', 'todayEarnings');
  ...
  return (
    <div className="flex min-h-screen flex-col items-center bg-[#F3F7EE] px-6 pt-16">
      ...
      <h1 className="mt-6 text-2xl font-extrabold text-gray-900">Delivery Completed</h1>
      <p className="mt-1 text-center text-sm text-gray-500">
        Order #{(delivery?.id ?? id ?? '').slice(0, 8)} successfully dropped off.
      </p>

      <div className="mt-6 w-full max-w-sm rounded-2xl bg-white p-4 shadow-sm">
        <div className="flex items-center gap-3">
          <span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 text-sm font-bold text-primary">
            {(delivery?.customer?.name ?? 'C').slice(0, 1).toUpperCase()}
          </span>
          <div>
        ...
        <div className="mt-3 flex items-center justify-between border-t border-gray-100 pt-3 text-sm">
          <div>
            <p className="text-xs text-gray-400">Completed Time</p>
            <p className="font-bold text-gray-900">{completedAt}</p>
          </div>
      ...
      <div className="mt-4 w-full max-w-sm rounded-2xl bg-[#F3F7EE] border border-primary/10 p-4">
        <h3 className="text-sm font-bold text-gray-900">Earnings Update</h3>
        <div className="mt-2 flex justify-between text-sm">
          <span className="text-gray-500">Delivery Fee Earned</span>
        ...
        <div className="mt-1 flex justify-between text-base font-bold">
          ...
          <span className="text-gray-900">₦{todaysTotal.toLocaleString()}</span>
        </div>
      </div>
      ...
      <div className="mt-6 w-full max-w-sm space-y-3 pb-10">
        {nextDeliveryId ? (
          <button
            type="button"
            onClick={() => navigate(`/rider/deliveries/${nextDeliveryId}`)}
